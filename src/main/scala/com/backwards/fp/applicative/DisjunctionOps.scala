...
object DisjunctionOps {
  implicit def disjunctionApplicative[L](implicit F: Functor[Disjunction[L, ?]]): Applicative[Disjunction[L, ?]] = new Applicative[Disjunction[L, ?]] {
    ...
    def <*>[A, R](ff: Disjunction[L, A => R])(fa: Disjunction[L, A]): Disjunction[L, R] = ff match {
      ...
      case RightDisjunction(f) => F.fmap(fa)(f)
    }
  }

  implicit def toApplicativeOps[L, A, R](disjunction: Disjunction[L, A => R])(implicit APP: Applicative[Disjunction[L, ?]]): ApplicativeOps[Disjunction[L, ?], A, R] =
