...
import tech.backwards.fp.demo.state.Signal._
...
object TrafficLightStateDemo extends App {
  ...
  def display(seq: Seq[Aspect -> Mode]): Signal => Signal =
    signal =>
  ...
  // Program
  val program: State[Signal, List[Signal]] /*IndexedStateT[Eval, Signal, Signal, List[Signal]]*/ =
    for {
      _  <- enable
      r0 <- get // Debugging
      _  <- halt
