...
object StateDemo extends App {
  def increment: State[Int, String] =
    State[Int, String] { state =>
      val a = state + 1
      (a, s"Result of increment is $a")
    }

  def double: State[Int, String] =
    State[Int, String] { state =>
      val a = state * 2
      (a, s"Result of double is $a")
    }

  def modTen: State[Int, String] =
    State[Int, String] { state =>
      val a = state % 10
  ...
  def genNumber: State[Int, String] /*IndexedStateT[Eval, Int, Int, String]*/ =
    for {
  ...
  val s: State[List[Int], Map[String, Int]] /*IndexedStateT[Eval, List[Int], List[Int], Map[String, Int]]*/ =
    for {
      a <- State(head[Int])
      b <- State(head[Int])
    } yield Map(
  ...
  val sOther: State[List[Int], Map[String, Int]] =
    State(head[Int]).flatMap(a => // flatMap calls "run" to thread through the state
      State(head[Int]).map( // map calls "run" to thread through the state
        b => Map("a" -> a, "b" -> b)
      )
    )
}