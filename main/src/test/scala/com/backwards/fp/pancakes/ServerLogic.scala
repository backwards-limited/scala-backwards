...
object ServerLogic extends StrictLogging {
  /**
   * Creating a queue is a Task; we bound the queue to 10 elements, not to overflow memory, and backpressure the websocket if too many ingredients are arriving
   */
  def bakePancakes(fryingPans: Int): Task[String Either Pipe[Task, PancakeIngredient, PancakeStatus]] =
  ...
  // Description a process which accumulates the incoming ingredients; when enough are ready, they are enqueued for frying;
  // once the frying process is free (see below), they will be dequeued and the actual frying will commence
  ...
  // Description of a process which dequeues ingredients from the given `fryingQueue`,
  // and fries them using up to `fryingPans` in parallel
  private def fry(fryingPans: Int, fryingQueue: Queue[Task, PortionIngredients]): Stream[Task, PancakeReady.type] =
    fryingQueue.dequeue.flatMap { portionIngredients =>
      ...
      // Assigning the number of pancakes to fry to each pan; distributing evenly as possible, and adding one extra pancake to the remaining ones
      val pancakesPerFryingPan: Seq[Int] =
        (1 to fryingPans).map(i => singlePanPancakesCount + (if (i <= extraPancakesCount) 1 else 0))
  ...
  // A process simulating the behavior of a single frying pan: we assume that the ingredients are ready;
  // frying one pancake takes one second
  private def fryingPan(count: Int): Stream[Task, PancakeReady.type] =
    Stream.awakeEvery[Task](1.second).map(_ => PancakeReady).take(count)

  // Data classes that are used by the streams

  private case class Ingredients(flourGrams: Int, milkLiters: Double, eggsCount: Int) {
    val + : PancakeIngredient => Ingredients = {
    ...
    val - : Ingredients => Ingredients =
      i => Ingredients(flourGrams - i.flourGrams, milkLiters - i.milkLiters, eggsCount - i.eggsCount)

    val * : Int => Ingredients =
      n => Ingredients(flourGrams * n, milkLiters * n, eggsCount * n)
  }
