...
/**
 * Any case class to any case class migration with default values
 */
object L19 extends App {
  ...
  implicit def hList[K <: Symbol, H, T <: HList](
    ...
    createMonoid(field[K](hMonoid.value.empty) :: tMonoid.empty)(
      (l, r) => field[K](hMonoid.value.combine(l.head, r.head)) :: tMonoid.combine(l.tail, r.tail)
    )
  ...
  implicit def genericMigration[
    ...
    targetRepr: LabelledGeneric.Aux[Target, TargetRepr],
    inter: hlist.Intersection.Aux[SourceRepr, TargetRepr, Common],
    dif: hlist.Diff.Aux[TargetRepr, Common, Added],
    monoid: Monoid[Added],
    prepend: hlist.Prepend.Aux[Added, Common, Unaligned],
    align: hlist.Align[Unaligned, TargetRepr]
   ): Migration[Source, Target] =
    a => targetRepr.from( // Create Target from Repr
