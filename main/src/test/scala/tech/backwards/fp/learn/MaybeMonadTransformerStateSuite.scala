...
class MaybeMonadTransformerStateSuite extends ScalaCheckSuite {
  ...
  property("MaybeT") {
    ...
    val example: MaybeT[State[List[String], *], Int] =
      MaybeT(State((xs: List[String]) => xs ++ List("bar") -> Nothing[Int]))

    /*assertEquals(
      MaybeT(State[List[String], *](_ ++ List("bar") -> Nothing[Int])).value.run(List("foo")),
      List("foo", "bar") -> Nothing[Int]
    )*/
  }
